...
public class BrowsepathXPathBuilder implements XPathBuilder {
    ...
    private String getExpForBrowsePath(String[] browsePath, Element parentNode) {
        ...
        // get exp that evaluated to all nodes whose BrowseName given by first parameter and reference the second parameter as parent (isConstraint)
    ...
    private Node findReferencedChild(Node parent, List<Node> childrenNodes) {
       return ((Element) parent).element("References").elements("Reference")
                .stream().filter(ref ->
                         hierarchyReferences.contains(ref.attributeValue("ReferenceType"))
      ...
      try{
          ...
          children.addAll(root.getRootElement().elements().stream()
                  .filter(e -> e.element("References") != null
                          && e.element("References").elements("Reference").stream().anyMatch(r -> r.getText().equals(uaNode.attributeValue("NodeId").replaceAll("\"", ""))
                           && hierarchyReferences.contains(r.attributeValue("ReferenceType"))
                           && Objects.equals(r.attributeValue("IsForward"), "false"))).collect(Collectors.toSet()));
      //for example return null if uaNode doesn't have the subnode "References", this will be interpreted in the Expr @uaChildren as mal argument Exception
      }catch(Exception e){
          return null;
      }
      return children;
